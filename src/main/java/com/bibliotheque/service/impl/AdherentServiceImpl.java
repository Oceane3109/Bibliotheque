...
public class AdherentServiceImpl implements AdherentService {
    ...
    public void decrementerLivresEmpruntesDomicile(Long adherentId) {
        Adherent adherent = getAdherentById(adherentId)
                .orElseThrow(() -> new EntityNotFoundException("Adhérent non trouvé avec l'ID: " + adherentId));
        Integer current = adherent.getLivresEmpruntesDomicile();
        if (current == null || current <= 0) {
            adherent.setLivresEmpruntesDomicile(0);
            adherentRepository.save(adherent);
            return;
        }
        if (current > 0) {
            adherent.setLivresEmpruntesDomicile(current - 1);
            adherentRepository.save(adherent);
        }
    }
    ...
    public void decrementerLivresEmpruntesSurPlace(Long adherentId) {
        Adherent adherent = getAdherentById(adherentId)
                .orElseThrow(() -> new EntityNotFoundException("Adhérent non trouvé avec l'ID: " + adherentId));
        Integer current = adherent.getLivresEmpruntesSurplace();
        if (current == null || current <= 0) {
            adherent.setLivresEmpruntesSurplace(0);
            adherentRepository.save(adherent);
            return;
        }
        if (current > 0) {
            adherent.setLivresEmpruntesSurplace(current - 1);
            adherentRepository.save(adherent);
        }
    }
