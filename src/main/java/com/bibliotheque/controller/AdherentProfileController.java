...
public class AdherentProfileController {
    ...
    public String profile(@AuthenticationPrincipal UserDetails userDetails, Model model) {
        ...
        var pretsActifs = adherent.getPrets().stream()
            .filter(pret -> pret.getEtatPret() != null && ("en_cours".equals(pret.getEtatPret()) || "en_retard".equals(pret.getEtatPret())))
            .toList();
        long nbDomicileActifs = pretsActifs.stream()
            .filter(pret -> normaliserTypePret(pret.getTypePret() != null ? pret.getTypePret().getNomTypePret() : null).contains("domicile"))
            .count();
        long nbSurPlaceActifs = pretsActifs.stream()
            .filter(pret -> normaliserTypePret(pret.getTypePret() != null ? pret.getTypePret().getNomTypePret() : null).contains("surplace"))
            .count();
        ...
        long totalDomicile = tousPrets.stream()
            .filter(pret -> normaliserTypePret(pret.getTypePret() != null ? pret.getTypePret().getNomTypePret() : null).contains("domicile"))
            .count();
        long totalSurPlace = tousPrets.stream()
            .filter(pret -> normaliserTypePret(pret.getTypePret() != null ? pret.getTypePret().getNomTypePret() : null).contains("surplace"))
            .count();
