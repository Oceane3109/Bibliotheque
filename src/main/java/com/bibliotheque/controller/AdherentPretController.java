...
public class AdherentPretController {
    ...
    public String emprunterLivre(@PathVariable Long exemplaireId, 
        ...
        String typePretNom = normaliserTypePret(typePret.getNomTypePret());
        if (typePretNom.contains("domicile")) {
        ...
        } else if (typePretNom.contains("surplace")) {
    ...
    public String reserverLivre(@PathVariable Long livreId,
        ...
        // Vérifier si l'adhérent a déjà une réservation active pour ce livre
        List<Reservation> reservationsExistantes = reservationService.getReservationsByAdherentAndLivre(adherent, livre);
        boolean aReservationActive = reservationsExistantes.stream().anyMatch(res -> {
            String etat = res.getEtatReservation();
            boolean etatActif = "en_attente".equalsIgnoreCase(etat)
                    || "disponible".equalsIgnoreCase(etat)
                    || "confirmee".equalsIgnoreCase(etat);
            if (!etatActif) {
                return false;
            }
            // Si la date de fin est passée, on considère la réservation terminée
            if (res.getDateFinPret() != null && res.getDateFinPret().isBefore(LocalDate.now())) {
                return false;
            }
            // Une réservation confirmée ne bloque que si un prêt actif existe encore pour ce livre
            if ("confirmee".equalsIgnoreCase(etat)) {
                return pretLivreService.getPretsByAdherent(adherent).stream().anyMatch(pret ->
                    pret.getExemplaire() != null
                        && pret.getExemplaire().getLivre() != null
                        && pret.getExemplaire().getLivre().getIdLivre() != null
                        && pret.getExemplaire().getLivre().getIdLivre().equals(livre.getIdLivre())
                        && pret.getEtatPret() != null
                        && ("en_cours".equals(pret.getEtatPret()) || "en_retard".equals(pret.getEtatPret()))
                );
            }
            return true;
        });
        ...
        String typePretNomReservation = normaliserTypePret(typePretOpt.get().getNomTypePret());
        if (typePretNomReservation.contains("domicile")) {
            if (pretLivreService.countPretsEnCoursDomicileByAdherent(adherent) >= adherent.getMaxLivresDomicile()) {
                redirectAttributes.addFlashAttribute("error", "Vous avez atteint votre limite de prêts à domicile.");
                return "redirect:/adherent/livre/" + livreId;
            }
        } else if (typePretNomReservation.contains("surplace")) {
            if (pretLivreService.countPretsEnCoursSurPlaceByAdherent(adherent) >= adherent.getMaxLivresSurplace()) {
                redirectAttributes.addFlashAttribute("error", "Vous avez atteint votre limite de prêts sur place.");
                return "redirect:/adherent/livre/" + livreId;
            }
    ...
    private String normaliserTypePret(String nomTypePret) {
        if (nomTypePret == null) {
            return "";
        }
        return nomTypePret.toLowerCase().replace(" ", "").replace("_", "");
    }
